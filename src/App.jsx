...
function calculateTotals(data) {
  const safeData = data || {};
  const usdRate = Number((safeData.settings && safeData.settings.usdRate) || 0);
  const incomes = Array.isArray(safeData.incomes) ? safeData.incomes : [];
  const expenses = Array.isArray(safeData.expenses) ? safeData.expenses : [];
  const investments = Array.isArray(safeData.investments) ? safeData.investments : [];

  const incomeCollected = incomes
    .filter((item) => item.status === "cobrado")
    .reduce((sum, item) => sum + convertToARS(item, usdRate), 0);
  ...
  const investmentTotal = investments.reduce((sum, item) => sum + convertToARS(item, usdRate), 0);
...
function buildInitialState() {
  return {
    ...
    expenses: FIXED_EXPENSES.map((name, index) => ({
      id: index + 1,
      name,
      category: categoryForFixedExpense(name),
      amount: 0,
      type: "fijo",
      paid: false,
      date: todayISO(),
    ...
    investments: [
      {
        id: 1,
        asset: "CEDEAR",
        ticker: "AAPL / GOOGL / META / NVDA",
        amount: 0,
        currency: "ARS",
        note: "Cartera actual en IOL",
      },
...
function loadStoredData() {
  if (!isStorageAvailable()) {
    return {
      ...
      message: "Modo seguro: el navegador no permitió guardado local.",
    };
  }
  ...
  try {
    ...
    if (!raw) {
      return { data: buildInitialState(), storageEnabled: true, message: "Guardado local activo." };
    }
    return {
      data: normalizeData(JSON.parse(raw)),
      storageEnabled: true,
      message: "Datos recuperados del dispositivo.",
    };
  } catch {
    return {
      data: buildInitialState(),
      storageEnabled: true,
      message: "No se pudo leer el guardado anterior. Se inició una copia limpia.",
    };
  }
}
...
function runSelfTests() {
  const testData = {
    incomes: [
      { amount: 100, currency: "USD", status: "cobrado" },
      { amount: 50000, currency: "ARS", status: "pendiente" },
    ],
    expenses: [
      { amount: 80000, type: "fijo", category: "Pagos dpto" },
      { amount: 20000, type: "variable", category: "Comida" },
    ],
...
export default function App() {
  ...
  function save(nextData, message = "Cambios guardados.") {
    ...
    setSystemMessage(
      didPersist
        ? message
        : "Cambios activos solo mientras la app esté abierta. Exportá un backup para no perderlos."
    );
  }
  ...
  function addIncome() {
    if (!incomeForm.client.trim() || !incomeForm.amount) return;
    const item = { ...incomeForm, id: Date.now(), amount: Number(incomeForm.amount) };
    save({ ...data, incomes: [item, ...data.incomes] }, "Ingreso agregado y guardado.");
    setIncomeForm(getIncomeForm());
  }

  function addExpense() {
    if (!expenseForm.name.trim() || !expenseForm.amount) return;
    const item = { ...expenseForm, id: Date.now(), amount: Number(expenseForm.amount) };
    save({ ...data, expenses: [item, ...data.expenses] }, "Gasto agregado y guardado.");
  ...
  function addGoal() {
    ...
    save({ ...data, goals: [item, ...data.goals] }, "Meta agregada y guardada.");
    setGoalForm(getGoalForm());
  }

  function addInvestment() {
    if (!investmentForm.ticker.trim() || !investmentForm.amount) return;
    const item = { ...investmentForm, id: Date.now(), amount: Number(investmentForm.amount) };
    save({ ...data, investments: [item, ...data.investments] }, "Inversión agregada y guardada.");
  ...
  function resetAllData() {
    const confirmed =
      typeof window === "undefined"
        ? true
        : window.confirm("Esto va a borrar tus datos actuales y volver a los datos de ejemplo. ¿Continuar?");
    if (!confirmed) return;
    clearStoredData();
  ...
  const nextStep =
    totals.fixed > 0 && totals.fixedCoverage < 100
...
function Header() {
  return (
    <header style={styles.header}>
      <p style={styles.headerEyebrow}>Finanzas freelance</p>
      <h1 style={styles.headerTitle}>Panel de Lil</h1>
      <p style={styles.headerText}>Ingresos variables, gastos reales, metas e inversiones en un solo lugar.</p>
    </header>
  );
}
...
function StatusBanner({ storageEnabled, message, updatedAt }) {
  return (
    <div style={storageEnabled ? styles.statusOk : styles.statusWarning}>
      <strong>{storageEnabled ? "Guardado activo" : "Modo seguro"}</strong>
      <p style={styles.statusText}>{message}</p>
      {updatedAt ? <p style={styles.statusDate}>Última actualización: {new Date(updatedAt).toLocaleString("es-AR")}</p> : null}
    </div>
  );
}
...
function Metrics({ totals }) {
  return (
    <section style={styles.metricsGrid}>
      <Metric icon="💰" label="Cobrado" value={formatMoney(totals.incomeCollected)} />
      <Metric icon="⏳" label="Pendiente" value={formatMoney(totals.incomePending)} />
      <Metric icon="🎯" label="Gastos" value={formatMoney(totals.totalExpenses)} />
      <Metric icon="📈" label="Libre" value={formatMoney(totals.available)} danger={totals.available < 0} />
    </section>
  );
}
...
function TabBar({ activeTab, onChange }) {
  return (
    <nav style={styles.tabs}>
      {TABS.map((tab) => (
        <button key={tab} type="button" style={activeTab === tab ? styles.tabActive : styles.tab} onClick={() => onChange(tab)}>
          {tab}
        </button>
      ))}
    </nav>
...
function PanelTab({ totals, expensesByCategory }) {
  return (
    <section style={styles.stack}>
      <Card title="Flujo general">
        <SimpleBarChart
          data={[
            { name: "Cobrado", value: totals.incomeCollected },
            { name: "Pendiente", value: totals.incomePending },
            { name: "Gastos", value: totals.totalExpenses },
            { name: "Libre", value: Math.max(0, totals.available) },
          ]}
        />
      </Card>
      <Card title="Gastos por categoría">
        {expensesByCategory.length ? <CategoryList data={expensesByCategory} /> : <Empty text="Cargá montos en tus gastos para ver el detalle." />}
      </Card>
    </section>
  );
}
    ...
    <section style={styles.stack}>
      <Card title="Nuevo ingreso">
        <div style={styles.formGrid}>
          <TextInput placeholder="Cliente" value={form.client} onChange={(value) => setForm({ ...form, client: value })} />
          <TextInput placeholder="Proyecto" value={form.project} onChange={(value) => setForm({ ...form, project: value })} />
          <TextInput type="number" placeholder="Monto" value={form.amount} onChange={(value) => setForm({ ...form, amount: value })} />
          <NativeSelect value={form.currency} onChange={(value) => setForm({ ...form, currency: value })} options={["ARS", "USD"]} />
          <NativeSelect value={form.status} onChange={(value) => setForm({ ...form, status: value })} options={["pendiente", "cobrado"]} />
          <TextInput type="date" value={form.date} onChange={(value) => setForm({ ...form, date: value })} />
          <label style={styles.checkboxLabel}>
            <input type="checkbox" checked={form.invoiced} onChange={(event) => setForm({ ...form, invoiced: event.target.checked })} />
            Factura realizada
          </label>
          <button type="button" style={styles.primaryButtonWide} onClick={addIncome}>+ Agregar ingreso</button>
        </div>
      </Card>
      ...
      <List
        ...
        render={(income) => (
          <Row
            key={income.id}
            title={`${income.client} · ${income.project || "Sin proyecto"}`}
            meta={`${formatMoney(income.amount, income.currency)} · ${income.status} · ${income.invoiced ? "facturado" : "sin factura"}`}
            onDelete={() => removeItem("incomes", income.id)}
            action={
              <button
                type="button"
                style={styles.smallButton}
                onClick={() => updateItem("incomes", income.id, { status: income.status === "cobrado" ? "pendiente" : "cobrado" })}
              >
                {income.status === "cobrado" ? "Pendiente" : "Cobrado"}
              </button>
            }
          />
        )}
  ...
  return (
    <section style={styles.stack}>
      <Card title="Nuevo gasto">
        <div style={styles.formGrid}>
          <TextInput placeholder="Nombre" value={form.name} onChange={(value) => setForm({ ...form, name: value })} />
          <TextInput type="number" placeholder="Monto ARS" value={form.amount} onChange={(value) => setForm({ ...form, amount: value })} />
          <NativeSelect value={form.category} onChange={(value) => setForm({ ...form, category: value })} options={EXPENSE_CATEGORIES} />
          <NativeSelect value={form.type} onChange={(value) => setForm({ ...form, type: value })} options={["fijo", "variable"]} />
          <TextInput type="date" value={form.date} onChange={(value) => setForm({ ...form, date: value })} />
          <button type="button" style={styles.primaryButton} onClick={addExpense}>+ Agregar</button>
        </div>
      </Card>
      ...
      <List
        ...
        render={(expense) => (
          <Row
            key={expense.id}
            title={`${expense.name} · ${expense.type}`}
            meta={`${formatMoney(expense.amount)} · ${expense.category} · ${expense.paid ? "pagado" : "pendiente"}`}
            onDelete={() => removeItem("expenses", expense.id)}
            action={<input type="checkbox" checked={expense.paid} onChange={(event) => updateItem("expenses", expense.id, { paid: event.target.checked })} />}
          />
        )}
      />
    </section>
  ...
  return (
    <section style={styles.stack}>
      <Card title="Nueva meta">
        <div style={styles.formGrid}>
          <TextInput placeholder="Meta" value={form.name} onChange={(value) => setForm({ ...form, name: value })} />
          <TextInput type="number" placeholder="Objetivo ARS" value={form.target} onChange={(value) => setForm({ ...form, target: value })} />
          <TextInput type="number" placeholder="Actual ARS" value={form.current} onChange={(value) => setForm({ ...form, current: value })} />
          <button type="button" style={styles.primaryButton} onClick={addGoal}>+ Agregar</button>
        </div>
      </Card>

      {items.map((goal) => {
        const progress = goal.target ? Math.min(100, Math.round((Number(goal.current || 0) / Number(goal.target)) * 100)) : 0;
        return (
          <Card key={goal.id} title={goal.name}>
            <ProgressBar value={progress} color="#53A3A6" />
            <p style={styles.muted}>{formatMoney(goal.current)} de {formatMoney(goal.target)} · {progress}%</p>
            <button type="button" style={styles.linkButton} onClick={() => removeItem("goals", goal.id)}>Eliminar</button>
          </Card>
        );
      })}
    </section>
...
function InversionesTab({ form, setForm, items, addInvestment, removeItem }) {
  return (
    <section style={styles.stack}>
      <Card title="Registro de inversiones">
        <div style={styles.formGrid}>
          <NativeSelect value={form.asset} onChange={(value) => setForm({ ...form, asset: value })} options={INVESTMENT_TYPES} />
          <TextInput placeholder="Ticker / Fondo" value={form.ticker} onChange={(value) => setForm({ ...form, ticker: value })} />
          <TextInput type="number" placeholder="Monto" value={form.amount} onChange={(value) => setForm({ ...form, amount: value })} />
          <NativeSelect value={form.currency} onChange={(value) => setForm({ ...form, currency: value })} options={["ARS", "USD"]} />
          <div style={styles.fullWidth}>
            <TextInput placeholder="Nota" value={form.note} onChange={(value) => setForm({ ...form, note: value })} />
          </div>
          <button type="button" style={styles.primaryButtonWide} onClick={addInvestment}>+ Agregar inversión</button>
        </div>
      </Card>

      <Card title="Consejo orientativo">
        <p style={styles.muted}>
          Perfil moderado: antes de aumentar riesgo, priorizá fondo de emergencia líquido. Luego combiná FCI de liquidez/renta fija para corto plazo, CEDEARs diversificados para dolarización parcial y acciones locales solo como porción más volátil.
        </p>
        <p style={styles.micro}>No es recomendación financiera personalizada. Revisar siempre costos, horizonte y riesgo antes de operar.</p>
      </Card>
      ...
      <List
        ...
        render={(investment) => (
          <Row
            key={investment.id}
            title={`${investment.asset} · ${investment.ticker}`}
            meta={`${formatMoney(investment.amount, investment.currency)} · ${investment.note || "sin nota"}`}
            onDelete={() => removeItem("investments", investment.id)}
          />
        )}
      />
    </section>
  ...
  return (
    <section style={styles.stack}>
      <Card title="Configuración rápida">
        <label style={styles.label}>Dólar de referencia para convertir USD a ARS</label>
        <TextInput type="number" value={data.settings.usdRate} onChange={(value) => save({ ...data, settings: { ...data.settings, usdRate: Number(value) } })} />
      </Card>

      <Card title="Backup y seguridad">
        <p style={styles.muted}>Recomendación: exportá un backup cada semana o antes de hacer cambios grandes.</p>
        <button type="button" style={styles.primaryButtonWide} onClick={exportBackup}>Descargar backup JSON</button>
        <button type="button" style={styles.secondaryButton} onClick={() => fileInputRef.current && fileInputRef.current.click()}>Importar backup</button>
        <input ref={fileInputRef} type="file" accept="application/json,.json" onChange={importBackup} style={{ display: "none" }} />
        <button type="button" style={styles.dangerButton} onClick={resetAllData}>Reiniciar datos</button>
      </Card>

      <Card title="Instalar en el celular como app">
        <ol style={styles.stepsList}>
          <li>Subí el proyecto a Vercel o Netlify.</li>
          <li>Abrí el link desde Safari en iPhone o Chrome en Android.</li>
          <li>Elegí “Agregar a pantalla de inicio”.</li>
          <li>Usala siempre desde ese ícono para mantener una experiencia estable.</li>
          <li>Exportá backups periódicos desde esta sección.</li>
        </ol>
        <p style={styles.micro}>Para que sea PWA completa, agregá un manifest.json y un ícono en la carpeta public del proyecto.</p>
      </Card>
    </section>
  );
}
...
function Metric({ icon, label, value, danger = false }) {
  return (
    <div style={styles.metricCard}>
      <div style={styles.metricTop}><span>{icon}</span><span>{label}</span></div>
      <strong style={danger ? styles.dangerValue : styles.metricValue}>{value}</strong>
    </div>
  );
}
...
function Card({ title, icon, children }) {
  return (
    <section style={styles.card}>
      {title ? <h2 style={styles.cardTitle}>{icon ? <span>{icon} </span> : null}{title}</h2> : null}
      {children}
    </section>
  );
}
        ...
        return (
          <div key={item.name} style={styles.barItem}>
            <div style={styles.barColumn}>
              <div title={formatMoney(item.value)} style={{ ...styles.bar, height, background: COLORS[index % COLORS.length] }} />
            </div>
  ...
  return (
    <div style={styles.stackSmall}>
      {data.map((item, index) => {
        ...
        return (
          <div key={item.name}>
            <div style={styles.categoryHeader}><span>{item.name}</span><strong>{formatMoney(item.value)}</strong></div>
            <ProgressBar value={percentage} color={COLORS[index % COLORS.length]} />
            <p style={styles.micro}>{percentage}% del total cargado</p>
          </div>
        );
      })}
    </div>
...
function List({ items, render }) {
  if (!items || !items.length) return <Empty text="Todavía no hay registros." />;
  return <div style={styles.stackSmall}>{items.map(render)}</div>;
}
  ...
  return (
    <div style={styles.row}>
      <div style={styles.rowContent}>
        ...
        <p style={styles.rowMeta}>{meta}</p>
        <button type="button" style={styles.linkButton} onClick={onDelete}>Eliminar</button>
      </div>
      {action ? <div style={styles.rowAction}>{action}</div> : null}
    </div>
  );
}
