...
const STORAGE_KEY = "finanzas-lil-v8";
...
function calculateTotals(data) {
  ...
  const totalIncome = incomeCollected + incomePending;
  ...
  const available = incomeCollected - paidExpenses;
  ...
  return { incomeCollected, incomePending, totalIncome, fixed, variable, totalExpenses, paidExpenses, pendingExpenses, available, fixedCoverage, investmentTotal };
}
...
function buildInitialState() {
  return {
    ...
    expenses: FIXED_EXPENSES.map((name, index) => ({ id: index + 1, name, category: defaultCategory(name), amount: 0, type: "fijo", paid: false, date: todayISO() })),
    goals: [
...
function getIncomeForm() {
  return { client: "", project: "", amount: "", currency: "ARS", status: "pendiente", invoiced: false, date: todayISO() };
}

function getExpenseForm() {
  return { name: "", category: "Comida", amount: "", type: "variable", paid: false, date: todayISO() };
}
...
function runSelfTests() {
  const testData = {
    ...
    expenses: [
      { amount: 80000, type: "fijo", category: "Pagos dpto", paid: true },
      { amount: 20000, type: "variable", category: "Comida", paid: false },
    ],
...
export default function App() {
  ...
  function addExpense() {
    ...
    save({ ...data, expenses: [{ ...expenseForm, id: Date.now(), name, amount }, ...data.expenses] }, "Gasto agregado y guardado.");
  ...
  return (
    <div style={styles.app}>
      <div style={styles.shell}>
        ...
        {activeTab === "Panel" && <PanelView totals={totals} expensesByCategory={expensesByCategory} />}
        {activeTab === "Ingresos" && <IncomeView form={incomeForm} setForm={setIncomeForm} items={data.incomes} addIncome={addIncome} removeItem={removeItem} updateItem={updateItem} />}
        {activeTab === "Gastos" && <ExpensesView form={expenseForm} setForm={setExpenseForm} items={data.expenses} addExpense={addExpense} removeItem={removeItem} updateItem={updateItem} />}
...
function MetricsGrid({ totals }) {
  ...
  return (
    <section style={styles.metricsGrid}>
      <div style={styles.metricExpandable}>
        <button type="button" style={styles.metricToggle} onClick={() => setShowIncomeDetails(!showIncomeDetails)}>
          <MetricCard icon="💎" label="Ingreso total" value={formatMoney(totals.totalIncome)} />
        </button>

        {showIncomeDetails ? (
          <div style={styles.metricDropdown}>
            <MetricCard icon="💰" label="Cobrado" value={formatMoney(totals.incomeCollected)} />
            <MetricCard icon="⏳" label="Pendiente por cobrar" value={formatMoney(totals.incomePending)} />
          </div>
        ) : null}
      </div>

      <div style={styles.metricExpandable}>
        <button type="button" style={styles.metricToggle} onClick={() => setShowExpenseDetails(!showExpenseDetails)}>
          <MetricCard icon="🎯" label="Gastos totales" value={formatMoney(totals.totalExpenses)} />
        </button>

        {showExpenseDetails ? (
          <div style={styles.metricDropdown}>
            <MetricCard icon="💸" label="Gastos pagos" value={formatMoney(totals.paidExpenses)} />
            <MetricCard icon="🧾" label="Gastos pendientes" value={formatMoney(totals.pendingExpenses)} />
          </div>
        ) : null}
      </div>

      <MetricCard icon="📈" label="Libre" value={formatMoney(totals.available)} danger={totals.available < 0} />
    </section>
  );
}
...
function PanelView({ totals, expensesByCategory }) {
  return (
    <section style={styles.stack}>
      <CardBox title="Flujo general">
        <SimpleBarChart data={[{ name: "Total", value: totals.totalIncome }, { name: "Cobrado", value: totals.incomeCollected }, { name: "Pendiente", value: totals.incomePending }, { name: "Libre", value: Math.max(0, totals.available) }]} />
      </CardBox>
      <CardBox title="Gastos por categoría">
        {expensesByCategory.length ? <CategoryList data={expensesByCategory} /> : <EmptyState text="Cargá montos en tus gastos para ver el detalle." />}
      </CardBox>
    </section>
  );
}

function IncomeView({ form, setForm, items, addIncome, removeItem, updateItem }) {
  return (
    <section style={styles.stack}>
      <CardBox title="Nuevo ingreso">
        <div style={styles.formGrid}>
          ...
          <TextInput inputMode="decimal" placeholder="Monto" value={form.amount} onChange={(value) => setForm({ ...form, amount: value })} />
          ...
          <label style={styles.checkboxLabel}><input type="checkbox" checked={form.invoiced} onChange={(event) => setForm({ ...form, invoiced: event.target.checked })} />Factura realizada</label>
          <button type="button" style={styles.primaryButtonWide} onClick={addIncome}>+ Agregar ingreso</button>
        </div>
      </CardBox>
      <RecordList items={items} render={(income) => <RecordRow key={income.id} title={`${income.client} · ${income.project || "Sin proyecto"}`} meta={`${formatMoney(income.amount, income.currency)} · ${income.status} · ${income.invoiced ? "facturado" : "sin factura"}`} onDelete={() => removeItem("incomes", income.id)} action={<button type="button" style={styles.smallButton} onClick={() => updateItem("incomes", income.id, { status: income.status === "cobrado" ? "pendiente" : "cobrado" })}>{income.status === "cobrado" ? "Pendiente" : "Cobrado"}</button>} />} />
    </section>
  );
}

function ExpensesView({ form, setForm, items, addExpense, removeItem, updateItem }) {
  return (
    <section style={styles.stack}>
      <CardBox title="Nuevo gasto">
        <div style={styles.formGrid}>
          <TextInput placeholder="Nombre opcional" value={form.name} onChange={(value) => setForm({ ...form, name: value })} />
          <TextInput inputMode="decimal" placeholder="Monto ARS" value={form.amount} onChange={(value) => setForm({ ...form, amount: value })} />
      ...
      <RecordList items={items} render={(expense) => <ExpenseEditableRow key={expense.id} expense={expense} updateItem={updateItem} removeItem={removeItem} />} />
    </section>
  );
}
  ...
  return (
    <div style={styles.editableRow}>
      <div style={styles.rowContent}>
        <p style={styles.rowTitle}>{`${expense.name} · ${expense.type}`}</p>
        <p style={styles.rowMeta}>{`${expense.category} · ${expense.paid ? "pagado" : "pendiente"}`}</p>
      </div>
      <div style={styles.expenseEditor}>
        <input type="text" inputMode="decimal" value={expense.amount} aria-label={`Monto de ${expense.name}`} onChange={(event) => updateItem("expenses", expense.id, { amount: parseAmount(event.target.value) })} style={styles.amountInput} />
        <label style={styles.paidInlineLabel}><input type="checkbox" checked={expense.paid} onChange={(event) => updateItem("expenses", expense.id, { paid: event.target.checked })} />Pagado</label>
        <button type="button" style={styles.linkButton} onClick={() => removeItem("expenses", expense.id)}>Eliminar</button>
      </div>
    </div>
  );
}
...
function GoalsView({ form, setForm, items, addGoal, removeItem }) {
  return (
    <section style={styles.stack}>
      <CardBox title="Nueva meta">
        <div style={styles.formGrid}>
          <TextInput placeholder="Meta" value={form.name} onChange={(value) => setForm({ ...form, name: value })} />
          <TextInput inputMode="decimal" placeholder="Objetivo ARS" value={form.target} onChange={(value) => setForm({ ...form, target: value })} />
          <TextInput inputMode="decimal" placeholder="Actual ARS" value={form.current} onChange={(value) => setForm({ ...form, current: value })} />
          <button type="button" style={styles.primaryButton} onClick={addGoal}>+ Agregar</button>
        </div>
      </CardBox>
      {items.map((goal) => {
        const target = parseAmount(goal.target);
        const progress = target ? Math.min(100, Math.round((parseAmount(goal.current) / target) * 100)) : 0;
        return <CardBox key={goal.id} title={goal.name}><ProgressBar value={progress} color="#53A3A6" /><p style={styles.muted}>{formatMoney(goal.current)} de {formatMoney(goal.target)} · {progress}%</p><button type="button" style={styles.linkButton} onClick={() => removeItem("goals", goal.id)}>Eliminar</button></CardBox>;
      })}
    </section>
  );
}
...
function InvestmentsView({ form, setForm, items, addInvestment, removeItem }) {
  return (
    <section style={styles.stack}>
      <CardBox title="Registro de inversiones">
        <div style={styles.formGrid}>
          ...
          <TextInput inputMode="decimal" placeholder="Monto" value={form.amount} onChange={(value) => setForm({ ...form, amount: value })} />
          <NativeSelect value={form.currency} onChange={(value) => setForm({ ...form, currency: value })} options={["ARS", "USD"]} />
          <div style={styles.fullWidth}><TextInput placeholder="Nota" value={form.note} onChange={(value) => setForm({ ...form, note: value })} /></div>
          <button type="button" style={styles.primaryButtonWide} onClick={addInvestment}>+ Agregar inversión</button>
        </div>
      </CardBox>
      <CardBox title="Consejo orientativo"><p style={styles.muted}>Perfil moderado: antes de aumentar riesgo, priorizá fondo de emergencia líquido. Luego combiná FCI de liquidez/renta fija para corto plazo, CEDEARs diversificados para dolarización parcial y acciones locales solo como porción más volátil.</p><p style={styles.micro}>No es recomendación financiera personalizada. Revisar siempre costos, horizonte y riesgo antes de operar.</p></CardBox>
      <RecordList items={items} render={(investment) => <RecordRow key={investment.id} title={`${investment.asset} · ${investment.ticker}`} meta={`${formatMoney(investment.amount, investment.currency)} · ${investment.note || "sin nota"}`} onDelete={() => removeItem("investments", investment.id)} />} />
    </section>
  );
}
...
const styles = {
  ...
  metricExpandable: { display: "flex", flexDirection: "column", gap: 10 },
  metricToggle: { border: 0, background: "transparent", padding: 0, textAlign: "left", cursor: "pointer" },
  metricDropdown: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, paddingLeft: 8 },
